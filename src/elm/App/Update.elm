...
update action model =
  case action of
    ChildArticleAction act ->
      let
        ...
        ( childModel, childEffects ) =
          Pages.Article.Update.update context act model.article
      in
    ...
    ChildConfigAction act ->
      let
        ( childModel, childEffects ) =
          Config.Update.update act model.config
    ...
    ChildEventAction act ->
      let
        ...
        ( childModel, childEffects ) =
          Pages.Event.Update.update context act model.events
      in
    ...
    ChildGithubAuthAction act ->
      let
        ...
        ( childModel, childEffects ) =
          Pages.GithubAuth.Update.update context act model.githubAuth
    ...
    ChildLoginAction act ->
      let
        ...
        ( childModel, childEffects ) =
          Pages.Login.Update.update context act model.login
    ...
    ChildUserAction act ->
      let
        ...
        ( childModel, childEffects ) =
          Pages.User.Update.update context act model.user
    ...
    SetActivePage page ->
      let
        ( page', nextPage ) =
          if model.user.name == Pages.User.Model.Anonymous then
            case page of
              App.GithubAuth ->
